...
vec4 CalculateLighting(Light light, vec3 surfacePosition, vec3 normal)
{
	...
	vec3 lightDirection = normalize(vec3(passFragmentInfo.TBN * vec3(light.Position)) - passFragmentInfo.TBN * passFragmentInfo.FragmentPosition);
	...
	{
		attenuation = 1.0;
		lightDirection = normalize(passFragmentInfo.TBN * light.Direction - passFragmentInfo.TBN * passFragmentInfo.FragmentPosition);
	...
	float radius = sqrt(1.0 / (light.Quadratic * MIN_LIGHT));
	if (distance > radius)
	{
		attenuation = 0.0;
	}

	return vec4((ambient + diffuse + specular) * attenuation, material.opacity);
}

void main(void)
{
	vec3 position = passFragmentInfo.FragmentPosition;
	vec3 normal = texture(normalMap[0], passFragmentInfo.UV).rgb;
	...
	//color = texture(normalmap, passFragmentInfo.UV);

	if (numDirectionalLights + numPointLights + numSpotLights == 0) {
		color = texture(diffuseMap[0], passFragmentInfo.UV) + texture(specularMap[0], passFragmentInfo.UV);
	}
}